...
server.post("/auth/register", async (request, response) => {
  ...
  if (user) {
    return response
      .status(400)
      .json({ message: "El usuario ya existe en nuestras base de datos" });
  }
...
server.post("/auth/login", async (request, response) => {
  ...
  if (!user) {
    return response
      .status(401)
      .json({ status: "Usuario no encontrado, credenciales invalidas" });
  }
  ...
  const token = jwt.sign({ id: user.id, email: user.email }, "CLAVE_SECRETA", {
    expiresIn: "1h",
  });
...
server.delete("/products/:id", authMiddleware, async (request, response) => {
  ...
  if (!deletedProduct) {
    return response
      .status(404)
      .json({ error: "No se encuentra el producto para borrar" });
  }
